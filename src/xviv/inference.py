...
def _build_dir_map() -> Dict:
    """
    Build direction enum → SV string map.
    ArgumentDirection.In/Out/InOut/Ref all confirmed via help(pyslang.ArgumentDirection).
    """
...
def _bit_width(typ) -> int:
    """
    Return flat bit-width of a pyslang Type.
    Type.bitWidth confirmed as readonly property via help(pyslang.Type).
    """
    try:
        return int(typ.bitWidth)        # ✅ Type.bitWidth confirmed
    except Exception:
        return 1
...
def parse_module(source_path: str,
    ...
    All pyslang API calls verified against help() output:
      SyntaxTree.fromFile(str)             ✅ confirmed static method
      Compilation()                        ✅ confirmed __init__ overload 1
      Compilation.addSyntaxTree(tree)      ✅ confirmed method
      Compilation.getAllDiagnostics()      ✅ confirmed method
      Compilation.getRoot()               ✅ confirmed method → RootSymbol
      RootSymbol.topInstances             ✅ confirmed readonly property
      InstanceSymbol.isModule             ✅ confirmed readonly property
      InstanceSymbol.name                 ✅ confirmed via Symbol
      InstanceSymbol.body                 ✅ confirmed readonly property → InstanceBodySymbol
      InstanceBodySymbol.portList         ✅ confirmed readonly property
      Scope.__iter__                      ✅ confirmed method (fallback)
      Symbol.kind.name                    ✅ confirmed (Symbol.kind + Enum.name)
      Symbol.name                         ✅ confirmed readonly property
      PortSymbol.direction                ✅ confirmed readonly property
      PortSymbol.type                     ✅ confirmed readonly property → Type
      Type.bitWidth                       ✅ confirmed readonly property
      ArgumentDirection.In/Out/InOut/Ref  ✅ confirmed enum values
    """
    global _DIR_MAP
    if _DIR_MAP is None:
    ...
    # ── Parse and compile ────────────────────────────────────────────────────
    tree = pyslang.SyntaxTree.fromFile(source_path)   # ✅ SyntaxTree.fromFile(str)
    comp = pyslang.Compilation()                       # ✅ Compilation()
    comp.addSyntaxTree(tree)                           # ✅ Compilation.addSyntaxTree

    if verbose:
        for d in comp.getAllDiagnostics():             # ✅ Compilation.getAllDiagnostics
            print(f"[pyslang] {d}", file=sys.stderr)

    root = comp.getRoot()                              # ✅ Compilation.getRoot → RootSymbol

    # Filter to modules only; topInstances may include interface instances
    instances = [
        i for i in root.topInstances                  # ✅ RootSymbol.topInstances
        if i.isModule                                  # ✅ InstanceSymbol.isModule
    ]
    ...
    if top:
        inst = next((i for i in instances if i.name == top), None)  # ✅ Symbol.name
        if inst is None:
    ...
    module_name: str = inst.name                       # ✅ Symbol.name
    ports: List[Port] = []

    # ── Primary: portList (preserves declaration order) ──────────────────────
    for sym in inst.body.portList:                     # ✅ InstanceSymbol.body
                                                       # ✅ InstanceBodySymbol.portList
        if sym.kind.name not in ("Port", "MultiPort"): # ✅ Symbol.kind (enum .name)
            continue
        direction = _DIR_MAP.get(sym.direction, "input")  # ✅ PortSymbol.direction
                                                          #    ArgumentDirection enum ✅
        bw    = _bit_width(sym.type)                   # ✅ PortSymbol.type → Type.bitWidth
    ...
    if not ports:
        ...
        for sym in inst.body:                          # ✅ Scope.__iter__
...
class PortGroup:
    ...
    mode:       str
    iface_name: str
    port_map:   Dict[str, Port] = field(default_factory=dict)   # logical → Port
...
def _score_group(ports: List[Port], iface_key: str) -> Tuple[int, Dict[str, Port]]:
    """Return (score, {logical → Port}) for fitting ports against an interface."""
...
def _find_prefix(port_name: str) -> str:
    """Strip the longest known signal suffix to recover the interface prefix."""
    lo      = port_name.lower()
...
def classify_ports(ports: List[Port],
                   verbose: bool = False) -> Tuple[List[PortGroup], List[Port]]:
    """
    Group ports by inferred prefix, score against every interface template,
    and return (groups, unassigned).
    """
    # ── Separate obvious clk/rst ports ──────────────────────────────────────
...
def generate_tcl(module_name: str,
                 groups: List[PortGroup],
                 unassigned: List[Port],
                 vlnv_v2: bool = False) -> str:
    """Render all PortGroups into a Vivado TCL string."""
