...
class ConfigTclBuilder:
	...
	def _create_bd_design(self, name: str, *, dir: str) -> None:
		params = filter(None, [f'-dir "{dir}"'])
	...
	def _get_bd_addr_segs(self, *, excluded: bool = False):
		params = filter(None, ["-excluded" if excluded else None])
	...
	def _create_core(self, name: str, *, dir: str, vlnv: str) -> None:
		params = filter(
			None,
			[
				f'-dir "{dir}"',
	...
	def _update_ip_catalog(self, *, rebuild: bool = False):
		params = filter(None, ["-rebuild" if rebuild else None])

		self._push(f"update_ip_catalog {' '.join(params)}")

	def _update_compile_order(self, *, fileset: str | None = None):
		params = filter(
			None,
			[
	...
	def _hsi__create_sw_design(self, name: str, *, proc: str | None = None, os: str | None = None):
		params = filter(
			None,
			[
	...
	def _hsi__generate_bsp(self, *, dir: str | None = None):
		params = filter(None, [f'-dir "{dir}"' if dir else None])
	...
	def _create_peripheral(self, *, vendor: str, library: str, name: str, version: str, dir: str):
		params = filter(None, [f'-dir "{dir}"'])

		self._push(f'create_peripheral "{vendor}" "{library}" "{name}" "{version}" {" ".join(params)}')

	def _add_peripheral_interface_ipx__find_open_core(
		self,
		interface: str,
		vlnv: str,
		*,
	...
	def _generate_peripheral_ipx__find_open_core(self, vlnv: str, *, force: bool = False):
		self._generate_peripheral(f'[ipx::find_open_core "{vlnv}"]', force=force)

	def _generate_peripheral(self, context: str, *, force: bool = False):
		params = filter(None, ["-force" if force else None])
	...
	def _ipgui__add_param(self, *, name: str, display_name: str, component: str, parent: str):
		params = filter(
			None,
			[
				f'-name "{name}"',
	...
	def _ipgui__get_pagespec(self, *, name: str, component: str):
		params = filter(
			None,
			[
				f'-name "{name}"',
	...
	def _ipx__edit_ip_in_project(self, component_xml_file: str, upgrade: bool, name: str, directory: str):
		params = filter(
			None,
			[
	...
	def _ipx__get_user_parameters(
		self,
		*,
		of_objects: str,
	):
		params = filter(None, [f"-of_objects {of_objects}"])

		self._push(f"ipx::get_user_parameters {' '.join(params)}")

	def _ipx__get_bus_interfaces(
		self,
		*,
		of_objects: str,
		name: str | None = None,
	...
	def _get_files(self, *, filter: str | None = None, of_objects: str | None = None) -> None:
		params = [
			f"-filter {filter}" if filter else None,
	...
	def _opt_design(self, *, directive: str | None = None):
		params = filter(
			None,
			[
				f'-directive "{directive}"' if directive else None,
	...
	def _place_design(self, *, directive: str | None = None):
		params = filter(
			None,
			[
				f'-directive "{directive}"' if directive else None,
	...
	def _phys_opt_design(self, *, directive: str | None = None):
		params = filter(
			None,
			[
				f'-directive "{directive}"' if directive else None,
	...
	def _route_design(self, *, directive: str | None = None):
		params = filter(
			None,
			[
				f'-directive "{directive}"' if directive else None,
	...
	def _write_checkpoint(self, file: str, *, force: bool = False):
		params = filter(
			None,
			[
				"-force" if force else None,
	...
	def _read_checkpoint(self, file: str, *, incremental: bool = False, cell: str | None = None):
		params = filter(
			None,
			[
	...
	def _write_verilog(self, file: str, *, mode: str, force: bool = False, sdf_anno: bool = False):
		params = filter(
			None,
			[
				"-force" if force else None,
	...
	def _write_hw_platform(self, file: str, *, force: bool = False, fixed: bool = False, include_bit: bool = False):
		params = filter(
			None,
			[
				"-force" if force else None,
	...
	def _file_delete(self, path: str, *, force: bool = False):
		params = filter(
			None,
			[
				"-force" if force else None,
	...
	def _override(
		...
		*,
		...
		self._push(f"rename {call} {rename_prefix}{call}\nproc {call} {{args}} " + "{")
	...
	def _fconfigure(self, fh: str, *, blocking: bool, buffering: str):
